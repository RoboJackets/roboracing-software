    ...
    if (approxC.size() == 7 && contourArea >= minContourArea && contourArea <= maxContourArea) {
      ...
      if ((rect.width * ratioMin <= rect.height && rect.width * ratioMax >= rect.height) ||
        ...
        if (rect.width > rect.height) {
          ...
          cv::putText(crop, std::to_string(matchSimilarity), cv::Point(rect.x, rect.y + 25), cv::FONT_HERSHEY_PLAIN, 2,
                      cv::Scalar(255, 0, 0), 2);

          if (matchSimilarity <= turnMatchSimilarityThreshold) {
            // find top point of the arrow and test its x location
            auto extremeY = std::minmax_element(c.begin(), c.end(),
                                                [](cv::Point const& a, cv::Point const& b) { return a.y < b.y; });
          ...
          cv::putText(crop, std::to_string(matchSimilarity), cv::Point(rect.x, rect.y + 50), cv::FONT_HERSHEY_PLAIN, 2,
                      cv::Scalar(255, 0, 0), 2);
...
int findStopBarFromHough(cv::Mat& frame, cv::Mat& output, double& stopBarAngle, double stopBarGoalAngle,
  ...
  for (size_t i = 0; i < lines.size(); i++) {
    ...
    if (fabs(stopBarGoalAngle - currAngle) <= stopBarGoalAngleRange) {  // allows some amount of angle error
      ...
      cv::putText(output, streamDist.str(), cv::Point(midpoint.x, edges.rows - dist / 2), cv::FONT_HERSHEY_PLAIN, 1,
                  cv::Scalar(0, 255, 0), 1);
...
void stopBar_callback(const sensor_msgs::ImageConstPtr& msg) {
  ...
  double stopBarAngle;
  int stopBarDetected =
      findStopBarFromHough(frame, debug, stopBarAngle, stopBarGoalAngle, stopBarGoalAngleRange, stopBarTriggerDistance,
                           stopBarTriggerDistanceRight, houghThreshold, houghMinLineLength, houghMaxLineGap);
...
void loadSignImages(std::string packageName, std::string fileName) {
  ...
  cv::resize(sign_forward, sign_forward,
             cv::Size(sign_forward.rows / 6, sign_forward.cols / 6));  // scale image down to make contour similar scale
...
int main(int argc, char** argv) {
  ...
  pubMove =
      nh.advertise<rr_iarrc::urc_sign>("/turn_detected", 1);  // publish the turn move for Urban Challenge Controller
