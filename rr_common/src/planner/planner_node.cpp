...
void processMap() {
    ...
    ROS_INFO_STREAM("Best path cost is " << plan.cost << ", collision = "
                                         << plan.has_collision); // update impasse state machine
    auto now = ros::Time::now();
    if (OK == reverse_state) {
        ...
        if (!plan.has_collision) { reverse_state = OK; }
...
int main(int argc, char **argv) {
    ...
    g_vehicle_model = std::make_unique<rr::BicycleModel>(ros::NodeHandle(nhp, "bicycle_model"), g_steer_model,
                                                         g_speed_model);
    std::string planner_type;
    assertions::getParam(nhp, "planner_type", planner_type);
    if (planner_type == "annealing") {
        g_planner = std::make_unique < rr::AnnealingOptimizer <
                    ctrl_dim >> (ros::NodeHandle(nhp, "annealing_optimizer"));
    }
    else if (planner_type == "hill_climbing") {
        g_planner = std::make_unique < rr::HillClimbOptimizer <
                    ctrl_dim >> (ros::NodeHandle(nhp, "hill_climb_optimizer"));
    }
    else {
    ...
    update_messages(0, 0);
    g_effector_tracker = std::make_unique<rr::EffectorTracker>(ros::NodeHandle(nhp, "effector_tracker"), speed_message,
                                                               steer_message);
