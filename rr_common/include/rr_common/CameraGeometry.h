...
class CameraGeometry {
  public:
     ...
     * @param camera_link_name URDF/tf link name for the camera (x forward). E.g.
     * "camera"
     * @param timeout Time after which this should give up
     * @return Whether the information was loaded within "timeout" seconds. If
     * false, user must publish information on the appropriate topics or make up
     * values.
     */
    ...
    /**
     * Return loaded camera position. Forward distance from base_footprint to
     * camera is GetCameraLocation().x
    ...
    /**
     * Return the loaded camera orientation as a (roll, pitch, yaw) triple.
     * Positive pitch is down, and positive yaw is counter-clockwise from
     * overhead.
    ...
    /**
     * Get angular distance from center row of image to another. For the
     * bottommost row, this is GetFOVVertical() / 2.
    ...
    /**
     * Get angular distance from center column of image to another. For the
     * leftmost column, this is GetFOVHorizontal() / 2.
    ...
    /**
     * Project the specified pixel location into world-space. Imagine drawing a
     * line from the focal point, through the image plane, and onto the ground
     * plane.
     * @param row Pixel y location
     * @param col Pixel x location
     * @return 0: bool, true if pixel is below the horizon.
     *         1: geometry_msgs::Point for world-space point. x < 10m, -10 < y <
     * 10, and z = 0.
     */
