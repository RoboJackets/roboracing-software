...
class BicycleModel {
 public:
   ...
   * @param distance_increment Distance between consecutive path points in rollout
   * @param max_speed Straight-line desired speed
   * @param max_steer_rate Rate at which the platform can change its steering angle
 ...
 private:
  ...
  /**
   * Calculate one distance-step (not timestep) of "simulated" vehicle motion. Bicycle-model
   * forward kinematics happens here.
