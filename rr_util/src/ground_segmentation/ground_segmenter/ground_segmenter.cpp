...
void GroundSegmenter::visualizePointCloud(const PointCloud::ConstPtr& cloud,
                                             const std::string& id) {
...
void GroundSegmenter::visualize(const std::list<PointLine>& lines,
...
void GroundSegmenter::getLines(std::list<PointLine> *lines) {
  ...
  for (i = 0; i < params_.n_threads; ++i) {
    ...
    thread_vec[i] = std::thread(&GroundSegmenter::lineFitThread, this,
...
void GroundSegmenter::lineFitThread(const unsigned int start_index,
...
pcl::PointXYZ GroundSegmenter::minZPointTo3d(const Bin::MinZPoint &min_z_point,
                                                const double &angle) {
  pcl::PointXYZ point;
...
void GroundSegmenter::assignCluster(std::vector<int>* segmentation) {
  ...
  for (unsigned int i = 0; i < params_.n_threads; ++i) {
    ...
    thread_vec[i] = std::thread(&GroundSegmenter::assignClusterThread, this,
...
void GroundSegmenter::assignClusterThread(const unsigned int &start_index,
                                             const unsigned int &end_index,
...
void GroundSegmenter::insertPoints(const PointCloud& cloud) {
  ...
  for (unsigned int i = 0; i < params_.n_threads - 1; ++i) {
    ...
    threads[i] = std::thread(&GroundSegmenter::insertionThread, this,
...
void GroundSegmenter::insertionThread(const PointCloud& cloud,
