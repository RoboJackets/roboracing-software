...
int main(int argc, char** argv) {
  ...
  */
  ...
  string pidMessage =
      "#" + buildPIDMessage(accelDrivePID) + " " + buildPIDMessage(decelDrivePID) + " " + buildPIDMessage(steeringPID);
  ...
  while (ros::ok()) {
    ...
    if (response.at(0) == '@') {
      ...
      string command = "$" + to_string(speed) + " " +
                       to_string(steeringAngle);  //#TODO: the firmware may need to be clearing the buffer properly
      sendMessage(command);
