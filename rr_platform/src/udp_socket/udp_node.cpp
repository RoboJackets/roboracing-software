...
int main(int argc, char** argv) {
    ...
    // IP address and port
    int udpPort = nhp.param(string("udp_port"), 8888);
    ...
    string steeringBoardIP = nhp.param(string("steering_ip_address"), string("192.168.20.56"));
    ...
    while (ros::ok()) {
        ...
        int i = driveBoardSocket->send("v=" + std::to_string(cmd_speed));  //
        ROS_INFO_STREAM("SENDING SPEED DATA: " << i);
