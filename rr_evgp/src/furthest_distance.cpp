...
class FurthestPoint : public rclcpp::Node {
  public:
    FurthestPoint(const std::string& name) : Node(name) {
        pos_sub_ = create_subscription<nav_msgs::msg::Odometry>("/odom", 10,
                                                                std::bind(&FurthestPoint::pos_callback, this, _1));
        map_sub_ = create_subscription<nav_msgs::msg::OccupancyGrid>("global_costmap/costmap", 10,
                                                                     std::bind(&FurthestPoint::map_callback, this, _1));
